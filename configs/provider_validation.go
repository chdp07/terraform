...
func validateProviderConfigs(call *ModuleCall, cfg *Config, noProviderConfig bool) (diags hcl.Diagnostics) {
	...
	for name, passed := range passedIn {
		...
		if !providerAddr.Provider.Equals(parentAddr.Provider) {
			diags = append(diags, &hcl.Diagnostic{
				...
				Detail: fmt.Sprintf("Cannot use configuration from %s for %s.\n", parentAddr, providerAddr) +
	...
	// If we have gotten this far without errors, verify that any passed in
	// configurations have the correct type.
